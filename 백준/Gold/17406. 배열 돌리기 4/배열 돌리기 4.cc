...
int n, m, k, a[104][104], b[104][104], ret = INF, r, c, s;
int visited[104][104], dir, sy, sx, ey, ex;
...
void go(int y, int x, int first) {
	...
	else if (!first && y == ey && x == ex) dir++;
	else if (!first && y == ey && x == sx) dir++;

	int ny = y + dy[dir];
	int nx = x + dx[dir];

	if (ny < sy || ny > ey || nx < sx || nx > ex) return;
